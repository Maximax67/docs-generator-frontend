...
export const userApi = {
  ...
  /**
   * Get saved variables for current user
   */
  async getSavedVariables(): Promise<Record<string, JSONValue>> {
    const response = await api.get<Record<string, JSONValue>>(`/variables/saved`);
    return response.data;
  },
  ...
  async updateSavedVariable(variableId: string, value: JSONValue): Promise<void> {
    await api.post(
      `/variables/${encodeURIComponent(variableId)}/save`, { value }
    );
  },
  ...
  async deleteSavedVariable(variableId: string): Promise<void> {
    await api.post(
      `/variables/${encodeURIComponent(variableId)}/forget`,
    );
  },
};
